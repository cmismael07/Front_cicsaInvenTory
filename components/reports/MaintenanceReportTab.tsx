    ...
    <div className="space-y-6">
                                    ...
                                    <button 
                                        onClick={() => {
                                            (async () => {
                                                    ...
                                                    // If the backend already returned an absolute URL, use it directly.
                                                    // Otherwise prefer our proxy endpoint so CORS headers are correct
                                                    // and we don't rely on window.location host/port.
                                                    const isAbsolute = raw.startsWith('http');
                                                    const isProxyPath = raw.startsWith('/api/files/mantenimientos') || raw.includes('/api/files/mantenimientos/');
                                                    // If backend returned an absolute storage URL (e.g. http://localhost:8000/storage/...),
                                                    // map it to our proxy so CORS headers are present.
                                                    let absoluteLooksLikeStorage = false;
                                                    if (isAbsolute) {
                                                        try {
                                                            const parsed = new URL(raw);
                                                            if (parsed.pathname.includes('/storage/mantenimientos') || parsed.pathname.includes('/storage/')) {
                                                                absoluteLooksLikeStorage = true;
                                                            }
                                                        } catch (e) {
                                                            // ignore
                                                        }
                                                    ...
                                                    let url = raw;
                                                    if ((!isAbsolute && !isProxyPath) || absoluteLooksLikeStorage) {
                                                        // Use proxy that serves maintenance files with CORS
                                                        url = backendOrigin + '/api/files/mantenimientos/' + encodeURIComponent((raw || '').split('/').pop() || '');
                                                    }
                                                try {
                                                    ...
                                                    setFileToView(url);
                                                    // infer type from extension when blob unavailable
                                                    const lower = (url || '').toLowerCase();
                                                    if (lower.endsWith('.pdf')) setFileToViewType('application/pdf');
                                                    else if (lower.match(/\.(png|jpg|jpeg|gif|bmp)$/)) setFileToViewType('image/*');
                                                    else setFileToViewType(null);
                                                }
                                            })();
                                        }}
        ...
        <Modal isOpen={!!fileToView} onClose={() => {
            ...
            setFileToView(null);
             ...
             <div className="p-4">
                {fileToView ? (
                    (fileToViewType === 'application/pdf' || (fileToView && fileToView.toLowerCase().endsWith('.pdf'))) ? (
