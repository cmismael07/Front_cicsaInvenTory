...
export const DisposalReportTab: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      {/* Toolbar */}
      <div className="flex flex-col md:flex-row justify-between items-end gap-4 bg-slate-50 p-4 rounded-lg border border-slate-200">
         <div className="w-full md:w-1/3">
            ...
            <input 
                ...
                className="w-full p-2 border border-slate-300 rounded text-sm bg-white outline-none focus:ring-2 focus:ring-red-500"
                value={filterText}
         ...
         <div className="flex items-center border rounded-lg overflow-hidden bg-white">
            <button onClick={() => setGrouping('NONE')} className={`px-4 py-2 text-xs font-medium flex items-center gap-2 ${grouping === 'NONE' ? 'bg-red-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
                <Layers className="w-3 h-3" /> Listado
            </button>
            <div className="w-px h-8 bg-slate-200"></div>
            <button onClick={() => setGrouping('TYPE')} className={`px-4 py-2 text-xs font-medium flex items-center gap-2 ${grouping === 'TYPE' ? 'bg-red-600 text-white' : 'text-slate-600 hover:bg-slate-50'}`}>
      ...
      <div className="bg-white border rounded-xl overflow-hidden shadow-sm">
         {loading ? (
             <div className="p-12 text-center text-slate-500">Cargando reporte de bajas...</div>
         ) : filteredBajas.length === 0 ? (
             <div className="p-12 text-center flex flex-col items-center text-slate-400">
                 <Trash2 className="w-12 h-12 mb-2 text-slate-200" />
             ...
             Object.entries(groupedData).map(([groupName, items]) => (
                 <div key={groupName}>
                     {grouping !== 'NONE' && (
                         <div className="bg-red-50 px-6 py-2 border-b border-red-100 flex items-center gap-2 font-semibold text-red-800 text-sm">
                             <Box className="w-4 h-4" /> {groupName} 
                             <span className="bg-white px-2 py-0.5 rounded-full text-xs border border-red-200 ml-1 text-red-600">{items.length}</span>
                         </div>
                     )}
                     <table className="min-w-full divide-y divide-slate-200">
                         <thead className="bg-slate-50">
                             <tr>
                                 <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Fecha Baja</th>
                                 <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Código</th>
                                 <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Equipo / Modelo</th>
                                 <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Detalle / Motivo</th>
                                 <th className="px-6 py-3 text-left text-xs font-semibold text-slate-500 uppercase">Usuario Resp.</th>
                                 <th className="px-6 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Evidencia</th>
                             </tr>
                         </thead>
                         <tbody className="divide-y divide-slate-100">
                             {items.map(item => (
                                 <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                                     ...
                                     <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">{item.equipo_codigo}</td>
                                     <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600">{item.equipo_modelo}</td>
                                     <td className="px-6 py-4 text-sm text-slate-600 max-w-md truncate" title={item.detalle}>{item.detalle}</td>
                                     <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{item.usuario_responsable}</td>
                                     <td className="px-6 py-4 whitespace-nowrap text-center">
                                         {item.archivo ? (
                                             ...
                                             <span className="text-slate-300 inline-block p-1.5" title="Sin archivo">
      ...
      <Modal isOpen={!!fileToView} onClose={() => setFileToView(null)} title="Evidencia de Baja">
         <div className="p-8 text-center bg-slate-50 rounded-lg">
             ...
             <div className="flex flex-col items-center justify-center p-6 border-2 border-dashed border-slate-300 rounded-lg bg-white">
                 ...
                 <p className="font-mono text-sm text-slate-700 font-bold break-all bg-slate-100 px-3 py-1 rounded">{fileToView}</p>
                 <p className="text-xs text-slate-400 mt-3">
                    Este es un visor simulado. En producción se descargaría o visualizaría el PDF/Imagen real.
                 </p>
             </div>
             <div className="mt-6">
                <button onClick={() => setFileToView(null)} className="px-5 py-2 bg-white border border-slate-300 text-slate-700 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors shadow-sm">
                    Cerrar
                </button>
             </div>
         </div>
